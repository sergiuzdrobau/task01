    ...
    dialog3 = $( "#dialog-form3" ).dialog({
      ...
      if ( valid ) {
        ...
        var suma2 = suma1/1.1;
          ...
          users['id'+id] = id; users['title'+id] = title.val(); users['brand'+id] = brand.val(); users['timeframe'+id] = timeframe.val(); users['win'+id] = win.val(); users['timeframeyear'+id] = timeframeyear.val(); users['sales'+id] = parseInt(sales.val());
...
data_to_array.push({"0": users['id'+i], "1" : users['title'+i], "2" : users['brand'+i], "3": users['sales'+i], "4" : (parseInt(users['sales'+i]) / 1.1).toFixed(2), "5" : users['timeframe'+i], "6" : users['timeframeyear'+i]});
      allData = {"getData" : data_to_array};
...
function delete_row(id){
  ...
  $( "#delete"+id ).on( "click", function() {
      ...
      var title_sel = ""; var brand_sel = ""; var timeframe_sel = ""; var timeframe_sel = ""; var timeframeyear_sel = ""; var sales_sel = 0; var sales2_sel = 0; var suma_sel1 = 0; var suma_sel2 = 0; nr_of_reg = 0;       
        users = localStorage.getItem('users');
          users = JSON.parse(users);
         delete users['id'+id]; delete users['title'+id]; delete users['brand'+id]; delete users['timeframe'+id]; delete users['win'+id]; delete users['timeframeyear'+id]; delete users['sales'+id];
         localStorage.setItem('users', JSON.stringify(users));
         users = localStorage.getItem('users');
        users = JSON.parse(users);  
...
function checkRegexp( o, regexp, n ) {
    ...
    function addUser() {
      ...
      if ( valid ) {
        ...
        users = {['id'+id] : id, ['title'+id] : title.val(), ['brand'+id] : brand.val(), ['timeframe'+id] : timeframe.val(), ['win'+id] : win.val(), ['timeframeyear'+id] : timeframeyear.val(), ['sales'+id] : parseInt(sales.val())}
      localStorage.setItem('users', JSON.stringify(users));
        }
        else {
          users = localStorage.getItem('users');
...
var suma_sel1 = 0; var suma_sel2 = 0;var nr_of_reg = 0;
  for (i=1;i<=nr_max;i++)
  {
    var element_to_delete = document.getElementById("added_div");
        if (element_to_delete){element_to_delete.parentNode.removeChild(element_to_delete);}
